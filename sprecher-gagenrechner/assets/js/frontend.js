(function () {
	...
	var DEFAULT_RESULT_MESSAGE = '<div class="src-result-empty"><strong>Projekt wählen</strong><p>Sobald links eine Projektart aktiv ist, erscheint hier der Preisrahmen mit Kerndaten, Rechten, Breakdown und Angebotsaktionen.</p></div>';
	var LOADING_RESULT_MESSAGE = '<div class="src-result-empty"><strong>Ergebnis wird aktualisiert</strong><p>Preisrahmen, Rechteübersicht und Angebotsbasis werden neu berechnet.</p></div>';
	var ERROR_RESULT_MESSAGE = '<div class="src-result-empty"><strong>Berechnung momentan nicht verfügbar</strong><p>Bitte prüfe Projektart, Rechteangaben und Umfang oder versuche es gleich erneut.</p></div>';
	var CASE_UI = {
	...
	function buildScopeGuidance(formData, ui) {
		...
		if (hasConcreteScope) { return ((CASE_UI[effectiveCase] && CASE_UI[effectiveCase].scopeCopy) || 'Die passenden Umfangsangaben richten sich nach Deiner Projektart.') + ' Trage hier nur die Angaben ein, die für diesen Fall tatsächlich benötigt werden.'; }
		switch (effectiveCase) {
			case 'werbung_mit_bild':
			case 'werbung_ohne_bild':
			case 'kleinraeumig':
				return 'In diesem Fall ist hier keine zusätzliche Mengeneingabe nötig. Die Empfehlung ergibt sich vor allem aus Variante, Gebiet, Laufzeit, Medium und aktiven Rechte-Erweiterungen.';
			case 'podcast':
				return 'Für diese Podcast-Auswahl ist hier kein zusätzlicher Umfang nötig. Entscheidend sind vor allem Variante und Nutzungseinordnung.';
			default:
				return ((CASE_UI[effectiveCase] && CASE_UI[effectiveCase].scopeCopy) || 'Für diesen Fall ist aktuell keine weitere Umfangseingabe nötig.') + ' Die Empfehlung stützt sich hier vor allem auf die bereits gewählten Projekt- und Rechteangaben.';
		}
	}
	...
	function updateGuidanceSummary(app, formData, ui) {
		...
		if (rightsNode) {
			rightsNode.innerHTML = '<strong>Aktueller Rechte-Status</strong><p>' + htmlEscape(rightsParts.length ? rightsParts.join(' · ') : 'Nach Projekt- und Variantenauswahl fassen wir hier Gebiet, Laufzeit, Medium und Rechte-Erweiterungen kompakt zusammen.') + '</p>';
		}
		if (journeyNode) {
			...
			journeyNode.innerHTML = '<div class="src-tower-journey-item"><span>Projekt</span><strong>' + htmlEscape(caseLabel) + '</strong></div>' +
				'<div class="src-tower-journey-item"><span>Rechte</span><strong>' + htmlEscape(rightsParts.length ? rightsParts.slice(0, 3).join(' · ') : 'Folgt nach der Auswahl') + '</strong></div>' +
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		function requestCalculation(reason) {
			...
			if (!state.validation.valid) { updateRedirectBanner(app, null); app.__sgkLastPayload = null; resultContainer.innerHTML = '<div class="src-result-empty"><strong>Für die Empfehlung fehlen noch Angaben</strong><p>' + htmlEscape(state.validation.message) + '</p></div>'; return; }
			abortPendingRequest();
			requestSequence += 1;
