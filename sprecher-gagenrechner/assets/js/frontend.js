(function () {
	...
	function renderKnowledgeAccordion() {
		...
		return '<section class="src-result-card src-result-card--knowledge"><div class="src-result-card-head"><strong>Wissenswertes</strong><p>Kurze Einordnung zu Rechten, Nutzung und Angebotspraxis.</p></div><div class="src-result-accordion">' + items.map(function (item, index) {
	...
	function renderResult(container, payload, formData) {
		...
		var extensionMarkup = extensionItems.length ? '<div class="src-result-micro-badges">' + extensionItems.map(function (item) { return '<span class="src-result-micro-badge">' + htmlEscape(item) + '</span>'; }).join('') + '</div>' : '<div class="src-result-note">Derzeit sind keine zusätzlichen Erweiterungen aktiv.</div>';
		...
		container.innerHTML = '' +
			...
			'<div class="src-result-grid src-result-grid--stack">' +
				...
				'<section class="src-result-card"><div class="src-result-card-head"><strong>Breakdown & Pakete</strong><p>Die zentralen Preisbausteine auf einen Blick.</p></div><div class="src-breakdown-section">' + renderBreakdownRows(breakdownItems) + '</div><div class="src-result-subsection"><strong>Paket-Alternativen</strong>' + renderPackageAlternatives(alternatives) + '</div></section>' +
