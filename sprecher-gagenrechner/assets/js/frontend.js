		...
		container.innerHTML = '' +
			'<div class="src-result-hero">' +
				...
				'<div class="src-result-meta-grid">' +
					...
					'<div class="src-result-meta-card"><span>Finale Spanne</span><strong>' + htmlEscape((totals.lower || '0,00 €') + ' / ' + (totals.mid || '0,00 €') + ' / ' + (totals.upper || '0,00 €')) + '</strong></div>' +
				'</div>' +
			'</div>' +
			'<div class="src-result-grid">' +
				'<section class="src-result-card"><div class="src-result-card-head"><strong>Projektzusammenfassung & Rechte</strong><p>Hauptfall, Untervariante, Rechte und Verwertung stehen direkt unter dem Preisfokus.</p></div>' + rightsMarkup + '<div class="src-result-subsection"><strong>Einordnung & Sonderfälle</strong>' + renderRouteSummary(routeTrace) + '</div></section>' +
				...
				'<section class="src-result-card"><div class="src-result-card-head"><strong>Angebotspositionen & Umfang</strong><p>Diese Positionen werden auch für Export und Angebotsvorschau verwendet.</p></div><div class="src-receipt-list src-receipt-list--detailed">' + positionMarkup + '<div class="src-receipt-total"><span>Kalkulationsbasis</span><span>' + htmlEscape(totals.mid || '0,00 €') + '</span></div></div></section>' +
				'<section class="src-result-card"><div class="src-result-card-head"><strong>Hinweise, Pakete & Export</strong><p>Sonderlogik, Paket-Alternativen und Textbausteine bleiben darunter gebündelt.</p></div><div class="src-result-subsection"><strong>Hinweise</strong>' + renderSimpleList(warnings.concat(notes), 'Aktuell liegen keine zusätzlichen Hinweise vor.') + '</div><div class="src-result-subsection"><strong>Paket-Alternativen</strong>' + packageMarkup + '</div></section>' +
			'</div>' +
