(function () {
	...
	function updateGuidanceSummary(app, formData, ui) {
		...
		if (journeyNode) {
			...
			journeyNode.innerHTML = '<h4 class="sgk-guidance-title">Projektstatus</h4><div class="src-tower-summary-list">' +
				...
				'<div class="src-tower-journey-item"><span>Umfang:</span><strong>' + htmlEscape(scopeSummary) + '</strong></div></div>';
		}
	}
	...
	function validateManualOffer(value, result) {
		if (value == null || value === '') { return { valid: false, message: 'Bitte hinterlege eine konkrete Angebotssumme.' }; }
		if (value <= 0) { return { valid: false, message: 'Die Angebotssumme muss innerhalb der berechneten Preisrange liegen.' }; }
		if (!result || !result.totals) { return { valid: false, message: 'Die Angebotssumme muss innerhalb der berechneten Preisrange liegen.' }; }
		if (value < Number(result.totals.lower || 0)) { return { valid: false, message: 'Der Betrag liegt unter der empfohlenen Preisrange.' }; }
		if (value > Number(result.totals.upper || 0)) { return { valid: false, message: 'Der Betrag liegt über der empfohlenen Preisrange.' }; }
		return { valid: true, message: 'Die Angebotssumme liegt innerhalb der berechneten Preisrange.' };
	}
	...
	function formatSummaryBlocks(text) {
		var parts = String(text || '').split(/\n+/).map(function (entry) { return String(entry || '').trim(); }).filter(Boolean);
		if (!parts.length) { return '<p>Die Zusammenfassung wird nach den Eingaben ergänzt.</p>'; }
		return '<div class="src-summary-stack">' + parts.map(function (entry) { return '<p>' + htmlEscape(entry) + '</p>'; }).join('') + '</div>';
	}
	...
	function renderResult(container, payload, formData) {
		...
		container.innerHTML = '' +
			'<div class="src-result-hero src-result-hero--stack">' +
				'<section class="src-result-card src-result-card--price"><div class="src-price-panel-head"><strong>LIVE</strong><span class="src-live-badge src-live-badge--panel"><span class="src-live-dot src-live-dot--pulse"></span>LIVE</span></div><div class="src-price-block"><div class="src-price-highlight-card"><span class="src-price-highlight-label">Ø Mittelwert</span><span class="src-price-highlight-value sgk-price src-count-animate" data-sgk-count-key="price-anchor" data-sgk-count-value="' + htmlEscape(result.totals && result.totals.mid ? result.totals.mid : 0) + '">' + htmlEscape(totals.mid || '0,00 €') + '</span></div><div class="src-price-range-card"><div class="src-price-range-row"><span class="src-price-range-meta">Preisrange (empfohlen)</span><div class="src-price-range-values"><strong class="src-count-animate" data-sgk-count-key="price-lower" data-sgk-count-value="' + htmlEscape(result.totals && result.totals.lower ? result.totals.lower : 0) + '">' + htmlEscape(totals.lower || '0,00 €') + '</strong><span>–</span><strong class="src-count-animate" data-sgk-count-key="price-upper" data-sgk-count-value="' + htmlEscape(result.totals && result.totals.upper ? result.totals.upper : 0) + '">' + htmlEscape(totals.upper || '0,00 €') + '</strong></div></div></div></div></section>' +
				'<section class="src-sidebar-card src-sidebar-card--soft"><div class="src-sidebar-card-head"><h3 class="src-sidebar-card-title">So setzt sich der Preis zusammen</h3></div><div class="src-sidebar-card-body"><div class="src-breakdown-section">' + renderBreakdownRows(breakdownItems) + '</div>' + (alternativesMarkup ? '<div class="src-sidebar-card-section"><strong>Alternative Pakete</strong>' + alternativesMarkup + '</div>' : '') + '</div></section>' +
				'<section class="src-manual-offer src-sidebar-card"><div class="src-sidebar-card-head"><h3 class="src-sidebar-card-title">Finales Angebot</h3></div><div class="src-sidebar-card-body"><div class="src-manual-offer-row"><input type="number" min="' + htmlEscape(result.totals && result.totals.lower ? result.totals.lower : 0) + '" max="' + htmlEscape(result.totals && result.totals.upper ? result.totals.upper : 0) + '" step="0.01" value="' + htmlEscape(result.manual_offer_total || '') + '" placeholder="z. B. 2450.00" data-sgk-manual-offer /><button type="button" class="src-btn-primary" data-sgk-sync-manual-offer>Betrag übernehmen</button></div><div class="src-manual-offer-range">Empfohlene Spanne: ' + htmlEscape(totals.lower || '0,00 €') + ' – ' + htmlEscape(totals.upper || '0,00 €') + '</div><div class="src-manual-offer-status ' + (manualValidation.valid ? 'is-valid' : 'is-invalid') + '">' + htmlEscape(manualValidation.message) + '</div><div class="src-storage-status">Aktuell: ' + htmlEscape(manualOffer) + '</div><div class="src-receipt-list src-receipt-list--detailed">' + positionMarkup + '</div></div></section>' +
				'<section class="src-sidebar-card src-sidebar-card--compact src-result-card--priority"><div class="src-sidebar-card-head"><h3 class="src-sidebar-card-title">Kompakte Summary</h3></div><div class="src-sidebar-card-body"><div class="src-summary-grid"><div class="src-summary-cell"><span>Projekt</span><strong>' + htmlEscape(caseLabel) + '</strong></div><div class="src-summary-cell"><span>Variante</span><strong>' + htmlEscape(variantLabel) + '</strong></div><div class="src-summary-cell"><span>Gebiet</span><strong>' + htmlEscape(rightsSummary.territory) + '</strong></div><div class="src-summary-cell"><span>Laufzeit</span><strong>' + htmlEscape(rightsSummary.duration) + '</strong></div><div class="src-summary-cell"><span>Medium</span><strong>' + htmlEscape(rightsSummary.media) + '</strong></div></div><button type="button" class="src-btn-secondary src-summary-details" data-sgk-action="copy-summary">Details anzeigen</button></div></section>' +
				'<section class="src-storage-panel src-sidebar-card"><div class="src-sidebar-card-head"><h3 class="src-sidebar-card-title">Gespeicherte Berechnungen</h3></div><div class="src-sidebar-card-body"><label for="sgk-saved-calculations">Berechnung auswählen</label><select id="sgk-saved-calculations" data-sgk-saved-list><option value="">Bitte auswählen</option></select><div class="src-storage-actions"><button type="button" class="src-btn-secondary" data-label="Berechnung speichern" data-feedback-label="Gespeichert" data-sgk-action="save"><i class="fa-solid fa-floppy-disk" aria-hidden="true"></i> Speichern</button><button type="button" class="src-btn-secondary" data-label="Berechnung laden" data-feedback-label="Geladen" data-sgk-action="load"><i class="fa-solid fa-folder-open" aria-hidden="true"></i> Laden</button><button type="button" class="src-btn-secondary" data-label="Berechnung löschen" data-feedback-label="Gelöscht" data-sgk-action="delete"><i class="fa-solid fa-trash" aria-hidden="true"></i> Löschen</button></div><div class="src-storage-status" data-sgk-storage-status>' + htmlEscape(storageAvailable() ? 'Berechnungen werden lokal in diesem Browser gespeichert.' : 'Lokales Speichern ist in dieser Umgebung nicht verfügbar.') + '</div></div></section>' +
				'<section class="src-result-accordion src-sidebar-card"><div class="src-sidebar-card-head"><h3 class="src-sidebar-card-title">Zusammenfassung</h3></div><div class="src-sidebar-card-body"><div class="src-summary-copy">' + formatSummaryBlocks(copyBlocks.summary) + '</div></div></section>' +
				renderKnowledgeAccordion() +
				'<button type="button" class="src-btn-primary src-sidebar-cta" data-sgk-action="open-pdf"><i class="fa-solid fa-file-pdf" aria-hidden="true"></i> Angebot als PDF speichern</button>' +
			'</div>';
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		function updateProgressIndicator() {
			...
			var bottomProgress = app.querySelector('[data-sgk-bottom-progress]');
			if (bottomProgress) {
				bottomProgress.classList.toggle('is-visible', !!caseKey);
			}
			var progressPercent = step4Complete ? 100 : ((activeStep - 1) / 4) * 100;
			var progressBar = app.querySelector('[data-sgk-progress-fill]');
			if (progressBar) { progressBar.style.width = progressPercent + '%'; }
			app.querySelectorAll('[data-sgk-progress-step]').forEach(function (dot) {
				var stepNum = parseInt(dot.getAttribute('data-sgk-progress-step'), 10);
