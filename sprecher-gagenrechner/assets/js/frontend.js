(function () {
	...
	function renderOfferPreview(result, formData, offerMeta) { var documentData = buildOfferDocumentData(result, formData, offerMeta); var exportPayload = documentData.exportPayload; var summary = exportPayload.summary || {}; var positions = exportPayload.positions || []; var rights = exportPayload.rights_overview || []; var notes = exportPayload.notes_for_offer || []; var legal = exportPayload.legal_notice || []; var alternatives = exportPayload.alternative_packages || []; var totalText = exportPayload.manual_offer_total != null ? currency(exportPayload.manual_offer_total) : 'Noch offen'; var midText = exportPayload.recommended_mid != null ? currency(exportPayload.recommended_mid) : '—'; var rangeText = exportPayload.recommended_range ? currency(exportPayload.recommended_range.lower) + ' – ' + currency(exportPayload.recommended_range.upper) : '—'; return { html: '<style>' + documentStyles() + '</style>' + '<div class="doc"><div class="sheet">' + '<div class="header"><div class="header-top"><div style="display:flex;gap:16px;align-items:flex-start;"><div class="logo">SGK</div><div class="headline"><p class="eyebrow">Professionelles Angebotsdokument</p><h1>Angebot Sprecherhonorar</h1><p>' + htmlEscape(formData.project_title || summary.display_title || summary.title || 'Projektangebot') + '</p></div></div><div style="text-align:right;"><span class="badge">' + htmlEscape(offerMeta.offer_number || 'Angebot') + '</span><p class="muted" style="margin-top:10px;">' + htmlEscape(formatDate(offerMeta.offer_date)) + '</p></div></div><div class="meta-grid"><div class="meta-card"><div class="section-title">Kunde</div><strong>' + htmlEscape(formData.customer_name || 'Nicht angegeben') + '</strong><small class="muted">' + htmlEscape(offerMeta.contact_name || 'Ansprechpartner optional') + '</small></div><div class="meta-card"><div class="section-title">Projekt</div><strong>' + htmlEscape(formData.project_title || summary.display_title || summary.title || 'Berechnung') + '</strong><small class="muted">Projektart: ' + htmlEscape(summary.display_title || summary.title || 'Projekt') + '</small></div><div class="meta-card"><div class="section-title">Absender</div><strong>' + htmlEscape(offerMeta.sender_company || 'Studio / Absender') + '</strong><small class="muted">' + htmlEscape([offerMeta.sender_email, offerMeta.sender_phone].filter(Boolean).join(' · ')) + '</small></div><div class="meta-card"><div class="section-title">Dokument</div><strong>' + htmlEscape(offerMeta.offer_number || 'Ohne Nummer') + '</strong><small class="muted">Stand ' + htmlEscape(formatDate(offerMeta.offer_date)) + '</small></div></div></div>' + '<div class="summary-grid"><div class="summary-card"><div class="section-title">Angebotsbasis</div><p>' + htmlEscape(offerMeta.intro_text || 'Vielen Dank für deine Anfrage. Nachfolgend erhältst du dein Angebot auf Basis der abgestimmten Nutzung und der aktuellen Preisermittlung.') + '</p></div><div class="summary-card"><div class="section-title">Errechnete Spanne</div><strong>' + htmlEscape(rangeText) + '</strong><small class="muted">Mittelwert ' + htmlEscape(midText) + '</small></div><div class="summary-card summary-card--total"><div class="section-title" style="color:rgba(255,255,255,.72)">Finale Angebotssumme</div><strong style="font-size:28px;display:block;">' + htmlEscape(totalText) + '</strong><small>' + htmlEscape(exportPayload.manual_offer_total != null ? 'Manuell gesetzt und als Angebotswert übernommen.' : 'Bitte vor PDF-Ausgabe final festlegen.') + '</small></div></div>' + '<div class="section positions"><div class="section-title">Angebotspositionen</div>' + positions.map(function (item) { var price = item.manuell_uebernommener_preis != null ? currency(item.manuell_uebernommener_preis) : currency(item.empfohlener_preis); return '<div class="position-row"><div><strong>' + htmlEscape(item.position_number + '. ' + item.titel) + '</strong><small>' + htmlEscape(item.beschreibung || '') + '</small><small>' + htmlEscape((item.kategorie || '') + (item.lizenzbezug ? ' · ' + item.lizenzbezug : '')) + '</small></div><div><strong>' + htmlEscape(price) + '</strong></div></div>'; }).join('') + '</div>' + '<div class="section rights"><div class="section-title">Nutzungsrechte & Lizenzen</div>' + (rights.length ? rights.map(function (item) { return '<div class="position-row"><div><strong>' + htmlEscape(item.title + (item.variant ? ' · ' + item.variant : '')) + '</strong><small>Laufzeit: ' + htmlEscape(item.duration) + ' · Territorium: ' + htmlEscape(item.territory) + ' · Medien: ' + htmlEscape(item.media) + '</small></div><div><span class="badge">Rechteblock</span></div></div>'; }).join('') : '<p class="muted">Keine zusätzlichen Rechteinformationen vorhanden.</p>') + '</div>' + '<div class="section notes"><div class="section-title">Hinweise & Anmerkungen</div>' + (notes.length ? notes.map(function (item) { return '<p style="padding:10px 0;border-top:1px solid #dbe6f1;">' + htmlEscape(item) + '</p>'; }).join('') : '<p class="muted">Keine zusätzlichen Hinweise.</p>') + '</div>' + (legal.length ? '<div class="section"><div class="section-title">Rechtlicher Hinweis</div>' + legal.map(function (item) { return '<p>' + htmlEscape(item) + '</p>'; }).join('') + '</div>' : '') + (alternatives.length ? '<div class="section"><div class="section-title">Optionale Paket-Alternativen</div>' + alternatives.map(function (item) { return '<div class="position-row"><div><strong>' + htmlEscape(item.label || 'Alternative') + '</strong></div><div><strong>' + htmlEscape(item.formatted_totals ? item.formatted_totals.mid : '') + '</strong></div></div>'; }).join('') + '</div>' : '') + '<div class="footer"><div>' + htmlEscape(offerMeta.footer_text || [offerMeta.sender_company, offerMeta.sender_email, offerMeta.sender_phone].filter(Boolean).join(' · ')) + '</div><div>Dieses Dokument basiert auf der aktuellen Preisermittlung im Sprecher-Gagenrechner. Interne Angaben werden im Kundendokument nicht ausgegeben.</div></div>' + '</div></div>', text: buildCopyBlocks(result, formData, offerMeta).mail, validation: validateManualOffer(exportPayload.manual_offer_total, result) }; }
	...
	function renderResult(container, payload, formData) {
		...
		var receiptItems = positions.length ? positions.map(function (item) { var price = item.formatted_prices && item.formatted_prices.manual ? item.formatted_prices.manual : ((item.formatted_prices && item.formatted_prices.mid) || '0,00 €'); return '<div class="src-receipt-item"><span>' + htmlEscape(item.titel) + '</span><div class="src-receipt-dots"></div><span>' + htmlEscape(price) + '</span></div>'; }).join('') : '<div class="src-receipt-item"><span>Basis</span><div class="src-receipt-dots"></div><span>' + htmlEscape(totals.mid || '0,00 €') + '</span></div>';
		container.innerHTML = '<div class="src-price-block"><div class="src-price-huge">' + htmlEscape(totals.mid || '0,00 €') + '<span>EUR</span></div><div class="src-price-range">Spanne laut VDS: ' + htmlEscape((totals.lower || '0,00 €') + ' – ' + (totals.upper || '0,00 €')) + '</div></div>' + '<div class="src-receipt-list">' + receiptItems + '<div class="src-receipt-total"><span>Kalkulationsbasis</span><span>' + htmlEscape(totals.mid || '0,00 €') + '</span></div></div>' + '<div class="src-rights-box"><strong>Lizenzumfang</strong>' + htmlEscape(rights.length ? rights.map(function (item) { return item.title + ' · ' + item.duration + ' · ' + item.territory; }).join(' | ') : 'Die Rechteübersicht wird nach der ersten Berechnung ergänzt.') + '</div>' + '<div class="src-result-actions"><button type="button" class="src-btn-primary" data-sgk-action="open-pdf">Angebot vorbereiten <span aria-hidden="true">→</span></button><div class="src-result-btn-grid"><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-label="Zusammenfassung kopieren" data-sgk-action="copy-summary">Zusammenfassung</button><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-label="Positionen kopieren" data-sgk-action="copy-positions">Positionen</button><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-label="Rechte kopieren" data-sgk-action="copy-rights">Rechte</button><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-label="Exportdaten kopieren" data-sgk-action="copy-json">Exportdaten</button></div></div>' + '<div class="src-inline-dark-panel src-manual-offer"><strong>Finale Angebotssumme</strong><div class="src-manual-offer-row"><input type="number" min="0" step="0.01" value="' + htmlEscape(result.manual_offer_total || '') + '" placeholder="z. B. 2450.00" data-sgk-manual-offer /><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-sgk-sync-manual-offer>Übernehmen</button></div><div class="src-manual-offer-status ' + (manualValidation.valid ? 'is-valid' : 'is-invalid') + '">' + htmlEscape(manualValidation.message) + '</div><div class="src-storage-status">Aktuell hinterlegt: ' + htmlEscape(manualOffer) + '</div></div>' + '<div class="src-storage-panel"><label for="sgk-saved-calculations">Gespeicherte Kalkulationen</label><select id="sgk-saved-calculations" data-sgk-saved-list><option value="">Bitte auswählen</option></select><div class="src-storage-actions"><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-label="Berechnung speichern" data-sgk-action="save">Speichern</button><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-label="Berechnung laden" data-sgk-action="load">Laden</button><button type="button" class="src-btn-secondary src-btn-secondary--dark" data-label="Berechnung löschen" data-sgk-action="delete">Löschen</button></div><div class="src-storage-status" data-sgk-storage-status>' + htmlEscape(storageAvailable() ? 'Deine Kalkulationen werden lokal im Browser gespeichert.' : 'Lokales Speichern ist in dieser Umgebung nicht verfügbar.') + '</div></div>' + '<div class="src-result-accordion"><div class="src-accordion-item is-open"><button type="button" class="src-accordion-btn" data-sgk-accordion-trigger><span>Aufschlüsselung & Hinweise</span></button><div class="src-accordion-content"><p>' + htmlEscape(copyBlocks.summary) + '</p></div></div><div class="src-accordion-item"><button type="button" class="src-accordion-btn" data-sgk-accordion-trigger><span>Wichtige Hinweise</span></button><div class="src-accordion-content"><p>' + htmlEscape((warnings.concat(notes).join(' · ')) || 'Aktuell liegen keine zusätzlichen Hinweise vor.') + '</p></div></div><div class="src-accordion-item"><button type="button" class="src-accordion-btn" data-sgk-accordion-trigger><span>Einordnung</span></button><div class="src-accordion-content"><p>' + htmlEscape(routeTrace.length ? routeTrace.map(function (item) { return routeLabel(item.step, item.label) + ': ' + prettifyRouteMessage(item.message); }).join(' · ') : 'Die Einordnung erscheint nach der Berechnung.') + '</p></div></div></div>';
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		function syncUI(options) {
			...
			var raw = serializeForm(form);
			var ui = deriveUiState(raw, cases);
			populateVariants(form, ui.effectiveCase);
