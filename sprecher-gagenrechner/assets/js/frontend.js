	...
	function renderResult(container, payload, formData) {
		...
		container.innerHTML = '' +
			'<div class="src-result-hero src-result-hero--stack">' +
				'<section class="src-result-card src-result-card--price"><div class="src-price-panel-head"><strong>LIVE RECHNUNG</strong><span class="src-live-badge src-live-badge--panel"><span class="src-live-dot src-live-dot--pulse"></span>LIVE</span></div><div class="src-price-block"><div class="src-price-highlight-card"><span class="src-price-highlight-value sgk-price src-count-animate" data-sgk-count-key="price-anchor" data-sgk-count-value="' + htmlEscape(result.totals && result.totals.mid ? result.totals.mid : 0) + '">' + htmlEscape(totals.mid || '0,00 €') + '</span><span class="src-price-highlight-label">Ø Mittelwert</span></div><div class="src-price-range-card"><div class="src-price-range-row"><span class="src-price-range-meta">Preisrange</span><div class="src-price-range-values"><strong class="src-count-animate" data-sgk-count-key="price-lower" data-sgk-count-value="' + htmlEscape(result.totals && result.totals.lower ? result.totals.lower : 0) + '">' + htmlEscape(totals.lower || '0,00 €') + '</strong><span>–</span><strong class="src-count-animate" data-sgk-count-key="price-upper" data-sgk-count-value="' + htmlEscape(result.totals && result.totals.upper ? result.totals.upper : 0) + '">' + htmlEscape(totals.upper || '0,00 €') + '</strong></div></div></div></div></section>' +
