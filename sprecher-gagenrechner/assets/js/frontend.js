	...
	function renderResult(container, payload, formData) {
		...
		container.innerHTML = '' +
			'<div class="src-result-hero src-result-hero--stack">' +
				...
				'<section class="src-manual-offer"><strong>Finales Angebot</strong><div class="src-manual-offer-row"><input type="number" min="0" step="0.01" value="' + htmlEscape(result.manual_offer_total || '') + '" placeholder="z. B. 2450.00" data-sgk-manual-offer /><button type="button" class="src-btn-secondary" data-sgk-sync-manual-offer>Betrag übernehmen</button></div><div class="src-manual-offer-status ' + (manualValidation.valid ? 'is-valid' : 'is-invalid') + '">' + htmlEscape(manualValidation.message) + '</div><div class="src-storage-status">Aktuell: ' + htmlEscape(manualOffer) + '</div><div class="src-receipt-list src-receipt-list--detailed">' + positionMarkup + '</div></section>' +
				'<section class="src-result-card src-result-card--priority"><div class="src-result-card-head"><strong>Kompakte Summary</strong></div><div class="src-keyvalue-list"><div class="src-keyvalue-row"><span>Projekt</span><strong>' + htmlEscape(caseLabel) + '</strong></div><div class="src-keyvalue-row"><span>Variante</span><strong>' + htmlEscape(variantLabel) + '</strong></div><div class="src-keyvalue-row"><span>Gebiet</span><strong>' + htmlEscape(rightsSummary.territory) + '</strong></div><div class="src-keyvalue-row"><span>Laufzeit</span><strong>' + htmlEscape(rightsSummary.duration) + '</strong></div><div class="src-keyvalue-row"><span>Medium</span><strong>' + htmlEscape(rightsSummary.media) + '</strong></div></div></section>' +
